...
// ۲ ساعت: چون کش لینک‌ها ۵۰ دقیقه نگه داشته می‌شه (SIGNED_URLS_CACHE_TTL_MS)،
// حتی لینکی که آخرین لحظه‌ی کش (دقیقه‌ی ۴۹) صادر شده حداقل ~۷۰ دقیقه اعتبار
// داره. با TTL یک‌ساعته، فقط ~۱۱ دقیقه می‌موند و عکس‌های lazy-load شده‌ی
// خواننده‌های کند 403 می‌گرفتن.
