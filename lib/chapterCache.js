  ...
  return Promise.all(
    keys.map((key) => {
      const command = new GetObjectCommand({
        Bucket: PARSPACK_BUCKET,
        Key: key,
        // به مرورگر/CDN می‌گه که این تصویر رو کش کنه؛ قبلاً این هدر ست
        // نمی‌شد و هر دانلود مستقیم از پارس‌پک انجام می‌شد حتی برای
        // تصاویری که قبلاً یک بار دیده شده بودن.
        ResponseCacheControl: `public, max-age=${SIGNED_URL_TTL_SECONDS}`,
      });
