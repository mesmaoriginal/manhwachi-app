...
function checkAndRecordGuestChapterRequest(guestId, ip, isNew = false) {
  ...
  if (isNew) {
    const r = checkAndRecordChapterRequest(`newguest-ip:${ip}`, {
  ...
  const r = checkAndRecordChapterRequest(`guest:${guestId}`);
