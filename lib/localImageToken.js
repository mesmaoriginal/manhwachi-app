...
// اگه از قبل تو .env ست نشده باشه، یک مقدار تصادفی موقع بالا اومدن پروسه
// می‌سازیم. تنها اثرش اینه که بعد از هر ری‌استارت سرور، لینک‌های قبلاً
// صادرشده باطل می‌شن (دقیقاً مثل presigned URL که هم TTL داره، پس این
// عارضه‌ی جدیدی نیست) - برای جلوگیری کامل از این عارضه، بهتره
// LOCAL_IMAGE_TOKEN_SECRET رو تو .env ثابت کنید (یک رشته‌ی تصادفی بلند،
// مثلاً با `openssl rand -hex 32`).
const SECRET =
  process.env.LOCAL_IMAGE_TOKEN_SECRET || crypto.randomBytes(32).toString("hex");

if (!process.env.LOCAL_IMAGE_TOKEN_SECRET) {
  console.warn(
    "[localImageToken] LOCAL_IMAGE_TOKEN_SECRET در .env تنظیم نشده - یک مقدار موقت و تصادفی ساخته شد؛ با هر ری‌استارت سرور، لینک‌های عکسِ قبلاً صادرشده باطل می‌شن (کاربر باید صفحه رو رفرش کنه)."
  );
}
